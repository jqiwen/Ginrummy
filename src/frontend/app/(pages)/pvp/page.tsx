...
function PlayerMark({ name }: { name: string }) {
  return <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#c8aa63]/40 bg-[#10271d] font-serif text-lg font-semibold uppercase text-[#d8bb71]">{name.charAt(0)}</span>;
}
...
function PrivateMatch() {
  ...
  async function invite(player: PublicPlayerProfile) {
    const sent = await runAction(player.id, () => sendInvite(player.username), `Invitation sent to ${player.username}.`);
    ...
    <main className="relative min-h-[calc(100vh-3.5rem)] overflow-hidden px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="relative mx-auto max-w-5xl">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div><p className="text-[10px] font-bold uppercase tracking-[0.28em] text-[#cbb270]">Private match</p><h1 className="mt-2 font-serif text-4xl font-semibold text-[#fff7df]">Invite a player to your table</h1><p className="mt-2 text-sm text-[#d8d1bf]/65">Search registered Gin Rummy players by public username.</p></div>
        ...
        <div className="grid items-start gap-5 lg:grid-cols-[1.15fr_0.85fr]">
          <Card className="rounded-sm border-[#b89b58]/45 bg-[#091912]/95 text-[#f5edd9] shadow-[0_28px_80px_rgba(0,0,0,0.45)] backdrop-blur-sm">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="player-search" className="text-[#eee4cb]">Search by username</Label>
                ...
                <p id="player-search-help" className="text-xs text-[#d8d1bf]/45">Results contain only public username and display name.</p>
              </div>
              <div className="min-h-28 rounded-sm border border-[#aa9159]/25 bg-[#06110d]/45 p-2" aria-live="polite">
                {query.trim().length < 2 && <div className="flex min-h-24 items-center justify-center text-sm text-[#d8d1bf]/40">Start typing a username</div>}
          ...
          <div className="space-y-5">
            ...
            <Card className="rounded-sm border-[#b89b58]/40 bg-[#091912]/95 text-[#f5edd9] backdrop-blur-sm">
              ...
              <CardContent>{sent.length === 0 ? <p className="py-5 text-sm text-[#d8d1bf]/45">No invitations sent.</p> : <ul className="space-y-3">{sent.map((invite) => <li key={invite.id} className="flex items-center gap-3 rounded-sm border border-[#aa9159]/25 bg-[#06110d]/55 p-3"><PlayerMark name={invite.recipient.username} /><div className="min-w-0 flex-1"><p className="truncate font-semibold text-[#fff4d5]">{invite.recipient.username}</p><p className="text-xs text-[#d8d1bf]/55">Waiting for a response…</p></div><Button size="sm" variant="ghost" disabled={Boolean(actionId)} onClick={() => void runAction(invite.id, () => cancelInvite(invite.id), "Invitation cancelled.")} className="text-[#d8d1bf]/60 hover:bg-red-950/30 hover:text-[#ffb4a7]">Cancel</Button></li>)}</ul>}</CardContent>
            </Card>
          </div>
        </div>
      </div>
